...
```{r}
# How good is this model? Use the metrics() fuction from the yardstick package
# to explore that question

metrics(x_1, dvote, estimate = pred_dvote) %>% 
  gt() %>% 
  tab_header("Model 1 Performance")

# Accuracy is the proportation of those rows which we got right
# Kap adjusts for how well a random guess would do
```


```{r}
# Trying to build a stronger model by adding another variable
...
metrics(x_2, dvote, estimate = pred_dvote) %>% 
  gt() %>% 
  tab_header("Model 1 Performance")

# This works a lot better! Kap is 56%. Accuracy is 20% better
# But is this actually better?
# What if you added social security or address to this model?
# Problem of overfitting?
# Look at summary(model_1) vs summary(model_2)- way more coefficients!
```
